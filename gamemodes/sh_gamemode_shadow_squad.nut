...
global asset FX_SHADOW_SPAWN_BEAM 					= $""
...
void function ShGameModeShadowSquad_Init()
{
	...
	AddCallback_EntitiesDidLoad( EntitiesDidLoad )
		...
		SURVIVAL_SetGameStateAssetOverrideCallback( ShadowSquadOverrideGameState )
...
void function OnGamePhaseChanged( entity player, int newVal )
{
	foreach( guy in GetPlayerArray() )
	{
		...
		if ( newVal == eShadowSquadGamePhase.FINAL_LEGENDS_DECIDED_EVAC_TIMER_STARTED )
		{
			...
			if ( IsAlive( guy ) && !IsPlayerShadowZombie( player ) )
			{
		...
		if ( newVal == eShadowSquadGamePhase.WINNER_DETERMINED )
		{
			...
			if ( IsAlive( guy ) && IsPlayerShadowZombie( guy ) && GetGlobalNetBool( "shadowsWonTheMode" ) )
			{
...
void function ServerCallback_ModeShadowSquad_AnnouncementSplash( int messageIndex, float duration )
{
	...
	switch( messageIndex )
	{
		...
		case eShadowSquadMessage.END_TIMER_EXPIRED:
			...
			break
		case eShadowSquadMessage.DISTRESS_SIGNAL_ACTIVATED:
			messageText = "#DISTRESS_SIGNAL_ACTIVATED_MESSAGE"
			subText = "#DISTRESS_SIGNAL_ACTIVATED_SUB"
			break
...
void function ShadowSquadOverrideGameState()
{
